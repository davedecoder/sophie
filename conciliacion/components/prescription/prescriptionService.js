...
// Actives
    ...
    function postPrescription(prescription, fecha, tipo) {
        var aux = {}, pre = [];
        var tm = fecha.toLocaleTimeString().split(" ")[0];
        var hora =(tm.length == 8) ? tm : "0"+tm;
        ...
        for (i in prescription)
        {
            ...
            aux.cronicoDatosFarma = (prescription[i].cronicoDatosFarma != undefined) ? parseInt(prescription[i].cronicoDatosFarma) : 0;
            aux.prescritoDatosFarma = (prescription[i].prescritoDatosFarma != undefined) ? parseInt(prescription[i].prescritoDatosFarma) : 0;     
